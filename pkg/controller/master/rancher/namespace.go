...
func (h *Handler) onNamespaceRemoved(_ string, namespace *corev1.Namespace) (*corev1.Namespace, error) {
	...
	kc, err := clientcmd.RESTConfigFromKubeConfig([]byte(rancherClusterConfig.KubeConfig))
	if err != nil {
		return nil, err
	}
	...
	harvesterConfigsGroupVersion := k8sschema.GroupVersionResource{Group: "harvesterconfigs.rke-machine-config.cattle.io", Version: "v1"}
	provisioningClusterGroupVersion := k8sschema.GroupVersionResource{Group: "provisioning.cattle.io", Version: "v1"}
