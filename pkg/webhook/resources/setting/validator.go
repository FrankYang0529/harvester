...
func (v *settingValidator) validateStorageNetwork(setting *v1beta1.Setting) error {
	...
	if err = checkNetworkOverlap(config, vmMigraionNetworkConfig); err != nil {
...
func (v *settingValidator) validateVMMigrationNetwork(setting *v1beta1.Setting) error {
	...
	if err = checkNetworkOverlap(storagetNetworkConfig, config); err != nil {
...
func checkNetworkOverlap(c1, c2 *networkutil.Config) error {
	if c1 == nil || c2 == nil {
		return nil
	}
	...
	for c1IP := range c1UsableIPAddresses {
		if _, ok := c2UsableIPAddresses[c1IP]; ok {
			return fmt.Errorf("the network configuration %s is overlapped with another %s", c1.ToString(), c2.ToString())
		}
	}
	return nil
}
